...
function [p_h,p_bag,p_blmax,p_h2d, ...
    p_bsap,p_bcr,p_bfrmax,p_bdead] = allom_lib_v4
...
switch traitp.hallom_mode{ipf}
    ...
    otherwise
        display(sprintf('Unknown H2D Allometry: %s',traitp.hallom_mode{ipf}));
        pause;
        return;
end
end
...
switch traitp.hallom_mode{ipf}
    ...
    otherwise
       display(sprintf('Unknown D-2-H Allometry: %s',traitp.hallom_mode{ipf}));
        pause;
        return;
end
end
...
l2f_ratio = traitp.l2f_ratio(ipf); % fine root biomass [kgC] per leaf biomass [kgC]
...
agb_fraction = traitp.agb_fraction(ipf); % Above-ground biomass fraction
...
dbcrdd = (1/agb_fraction-1).*dbagdd;
...
d2bl1_ad    = traitp.d2bl1_ad(ipf);   %0.0419;
d2bl2_ad    = traitp.d2bl2_ad(ipf);   %1.56;
d2bl3_ad    = traitp.d2bl3_ad(ipf);   %0.55;
rho      = traitp.wood_density(ipf);
dbh_maxh = traitp.dbh_maxh(ipf);
c2b       = traitp.c2b(ipf);
d_adult = traitp.d_adult(ipf);
d_sap   = traitp.d_sap(ipf);
...
elseif(dbh>=d_sap && dbh<d_adult)
    ...
    dblsapdd = d2bl1_sap * d2bl2_sap * d_sap.^(d2bl2_sap-1.0)./c2b;
    dbladdd  = d2bl1_ad * d2bl2_ad * d_adult.^(d2bl2_ad-1.0) * rho.^d2bl3_ad;
    
    [blmax,dblmaxdd] = cspline(d_sap,d_adult,blsap,blad,dblsapdd,dbladdd,dbh);
    
elseif(dbh>=d_adult && dbh<dbh_maxh)
    blmax = d2bl1_ad * dbh.^d2bl2_ad * rho.^d2bl3_ad;
    dblmaxdd = d2bl1_ad*d2bl2_ad * dbh.^(d2bl2_ad-1.0) * rho.^d2bl3_ad;
else
...
d2bl1_ad     = traitp.d2bl1_ad(ipf);
d2bl2_ad     = traitp.d2bl2_ad(ipf);
c2b       = traitp.c2b(ipf);
...
if(dbh>=d_adult)
    blmax    = d2bl1_ad.*dbh.^d2bl2_ad ./ c2b;
    dblmaxdd = d2bl1_ad.*d2bl2_ad.*dbh.^(d2bl2_ad-1.0) ./ c2b;
else
...
elseif(dbh_eff>=d_sap && dbh_eff<d_adult)
    ...
    dblsapdd = d2bl1_sap * d2bl2_sap * d_sap.^(d2bl2_sap-1.0)./c2b;
    [h_adult,~] = f_h(d_adult,traitp,ipf);
    [~,dddh]    = f_h2d(h_adult,traitp,ipf);
    [~,dhdd]    = f_h(d_adult,traitp,ipf);
    ddeffdd     = dddh * dhdd;
    dbladdd = d2bl1_ad.*d2bl2_ad.*d_adult.^(d2bl2_ad-1.0).*ddeffdd./c2b;
    [blmax,dblmaxdd] = cspline(d_sap,d_adult,blsap,blad,dblsapdd,dbladdd,dbh_eff);
    
else
    blmax       = d2bl1_ad.*dbh_eff.^d2bl2_ad./c2b;
    [~,dhdd]    = f_h(dbh,traitp,ipf);
    ddeffdd     = ddbhedh * dhdd;
    dblmaxdd    = d2bl1_ad.*d2bl2_ad.*dbh_eff.^(d2bl2_ad-1.0).*ddeffdd./c2b;
...
if(dbh>=d_adult && dbh<=dbh_maxh)
    blmax    = d2bl1_ad.*dbh.^d2bl2_ad./c2b;
    dblmaxdd = d2bl2_ad.*d2bl1_ad.*dbh.^(d2bl2_ad-1)./c2b;
elseif(dbh>dbh_maxh)
    ...
    blmax    = d2bl1_ad_small.*dbh.^d2bl2_ad_small ./ c2b;
    dblmaxdd = d2bl1_ad_small.*d2bl2_ad_small.*dbh.^(d2bl2_ad_small-1)./ c2b;
end
end
...
d2h1_ad     = traitp.d2h1_ad(ipf);      % (alias)
d2h2_ad     = traitp.d2h2_ad(ipf);      % (alias)
d2h3_ad     = traitp.d2h3_ad(ipf);      % (alias)
...
if(dbh>0.5*dbh_maxh)
    ...
    dbh0=0.5*dbh_maxh;
    h  = exp( d2h1_ad - eclim + d2h2_ad*log(dbh0) + d2h3_ad*log(dbh0).^2.0 );
    while(dbh0<dbh)

        fl = 1./(1+exp(-k*(dbh0-dbh_maxh)));
        ae = d2h1_ad-eclim;
        dhpdd = exp(ae).*( d2h3_ad.*2.0.*dbh0.^(d2h2_ad-1.0).*log(dbh0).* ...
            exp(d2h3_ad.*log(dbh0).^2) + d2h2_ad.*dbh0.^(d2h2_ad-1.0).* ...
            exp(d2h3_ad.*log(dbh0).^2.0) );
        dhdd = dhpdd*(1-fl);
        dbh0 = dbh0+ddbh;
        h    = h+ddbh*dhdd;
    end
...
%    display('necessary for production runs');
    ...
    h  = exp( d2h1_ad - eclim + d2h2_ad*log(dbh) + d2h3_ad*log(dbh).^2.0 );
end
...
%    dbh_maxh = exp(((d2h2_ad^2 - ...
%        4*(-log(h_max)+d2h1_ad-eclim)*d2h3_ad)^(1/2)-d2h2_ad)/(2*d2h3_ad));
% Logistic function
...
ae = d2h1_ad-eclim;
dhpdd = exp(ae).*( d2h3_ad.*2.0.*dbh.^(d2h2_ad-1.0).*log(dbh).* ...
    exp(d2h3_ad.*log(dbh).^2) + d2h2_ad.*dbh.^(d2h2_ad-1.0).* ...
    exp(d2h3_ad.*log(dbh).^2.0) );
dhdd = dhpdd*(1-fl);
...
h = d2h1_ad.*(1 - exp(d2h2_ad.*dbh.^d2h3_ad));
%h = h_max - h_max (exp(a.*dbh.^b));
%f(x) = -h_max*exp(g(x))
%g(x) = a*dbh^b
%d/dx f(g(x) = f'(g(x))*g'(x) = -a1*exp(a2*dbh^a3) * a3*a2*dbh^(a3-1)
%
%    h = -d2h3_ad.*exp(d2h1_ad.*dbh.^d2h2_ad).*d2h2_ad*d2h1_ad*dbh.^(d2h2_ad-1);
dhdd = -d2h1_ad*exp(d2h2_ad*dbh.^d2h3_ad) .* d2h3_ad.*d2h2_ad.*dbh.^(d2h3_ad-1);
end
...
% traitp.d2h1_ad: the intercept parameter (however exponential of the fitted log trans)
% traitp.d2h2_ad: the slope parameter
% return:
% h: total tree height [m]
% =========================================================================

d2h1_ad     = traitp.d2h1_ad(ipf);
d2h2_ad     = traitp.d2h2_ad(ipf);
dbh_maxh = traitp.dbh_maxh(ipf);
k      = 0.25;  % Stiffness of the logistic cap
ddbh     = 0.1; % 1-mm
...
if(dbh>0.5*dbh_maxh)
    dbh0=0.5*dbh_maxh;
    h = d2h1_ad.*dbh0.^d2h2_ad;
    while(dbh0<dbh)
        fl = 1./(1+exp(-k*(dbh0-dbh_maxh)));
        dhdd = (d2h2_ad*d2h1_ad).*dbh0.^(d2h2_ad-1).*(1-fl);
        h    = h+ddbh*dhdd;
        dbh0 = dbh0+ddbh;
    end
%    display('A request for a height calculation near hmax with chave');
...
%    display('necessary');
    ...
    h = d2h1_ad.*dbh.^d2h2_ad;
end

% The diameter at maximum height
%    dbh_maxh = (hmax./d2h1_ad).^(1./d2h2_ad);
% Logistic function
fl = 1./(1+exp(-k*(dbh-dbh_maxh)));
% Derivative of logistic function wrt dbh
dhdd = (d2h2_ad*d2h1_ad).*dbh.^(d2h2_ad-1).*(1-fl);
end
...
% your pft of interest, you set d_adult = 0 and d_sap = 0 in your
% parameter file.
% -------------------------------------------------------------------------
...
elseif(dbh>=d_sap && dbh<d_adult)
    ...
    dhdd_sap = d2h1_sap * d2h2_sap * d_sap^(d2h2_sap-1.0);
    dhdd_ad  = d2h1_ad * 10.0^d2h2_ad * d_adult^(d2h1_ad-1.0);
    h_sap    = d2h1_sap * d_sap^d2h2_sap;
    h_adult  = 10.0^(log10(d_adult) * d2h1_ad + d2h2_ad);
    [h,dhdd] = cspline(d_sap,d_adult,h_sap,h_adult,dhdd_sap,dhdd_ad,dbh);
    
elseif(dbh>=dbh_hmax)
    h    = 10.^(log10(dbh_hmax)*d2h1_ad+d2h2_ad);
    dhdd = 0;
else
    h    = 10.^(log10(dbh)*d2h1_ad+d2h2_ad);
    dhdd = d2h1_ad.*10.^d2h2_ad.*dbh.^(d2h1_ad-1.0);
end
...
eroot = (-d2h2_ad + sqrt(d2h2_ad^2 + 4*log(h.*exp(-ar))*d2h3_ad))./(2*d2h3_ad);
dbhe = exp(eroot);

% How about just inverting the derivative at phi? without asyms
eroot = (-d2h2_ad + sqrt(d2h2_ad^2 + 4*log(h.*exp(-ar))*d2h3_ad))./(2*d2h3_ad);
dbh1 = exp(eroot);
dhpdd = exp(ar).*( d2h3_ad.*2.0.*dbh1.^(d2h2_ad-1.0).*log(dbh1).* ...
    exp(d2h3_ad.*log(dbh1).^2) + d2h2_ad.*dbh1.^(d2h2_ad-1.0).* ...
    exp(d2h3_ad.*log(dbh1).^2.0) );
...
dbh = (log(1.0-h/d2h1_ad)./d2h2_ad).^(1.0/d2h3_ad);
ddbhdh = -((log(1-h/d2h1_ad)/d2h2_ad).^(1/d2h3_ad-1))./(d2h2_ad*d2h3_ad*(d2h1_ad-h));
end
...
%h = a1.*dbh.^a2;
dbh = (h/d2h1_ad).^(1/d2h2_ad);
%    dbh = (1/a1).^(1/a2).*h^(1/a2);
ddbhdh = (1/d2h2_ad).*(1/d2h1_ad).^(1/d2h2_ad).*h^(1/d2h2_ad-1.0);
end
...
elseif(h>=h_sap && h<h_adult)
    ...
    dddh_ad  = 1.0/(d2h1_ad*10^d2h2_ad*d_adult^(d2h1_ad-1.0));
    [dbh,ddbhdh] = cspline(h_sap,h_adult,d_sap,d_adult,dddh_sap,dddh_ad,h);
    
elseif(h<h_max)

    dbh    = 10.^((log10(h)-d2h2_ad)./d2h1_ad);
    ddbhdh = 1.0/(d2h1_ad.*10.^d2h2_ad.*dbh.^(d2h1_ad-1.0));
else
    dbh    = 10.^((log10(h_max)-d2h2_ad)./d2h1_ad);
